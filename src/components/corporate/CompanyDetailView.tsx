      ...
      <div className="relative overflow-hidden rounded-3xl border border-border/30 bg-card shadow-xl">
        ...
        <div className="relative p-6 sm:p-8">
          <div className="flex items-center justify-between mb-6">
            <Button variant="ghost" size="sm" className="gap-2 rounded-xl text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-all" onClick={onBack}>
              <ArrowRight className="h-4 w-4" />
              <span className="text-sm">{t('corporateMgmt.back')}</span>
            </Button>
            {company.is_active ? (
              ...
              <Badge variant="secondary" className="text-xs px-4 py-1.5 rounded-full">{t('corporateMgmt.inactive')}</Badge>
            )}
          </div>
          <div className="flex flex-col sm:flex-row items-start gap-5 mb-8">
            <div className="relative">
              {company.logo_url ? (
                <img src={company.logo_url} alt={company.name} className="h-16 w-16 rounded-2xl object-cover ring-2 ring-border/40 shadow-lg" />
              ) : (
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-foreground mb-1" style={{ fontFamily: "'Plus Jakarta Sans', 'IBM Plex Sans Arabic', sans-serif" }}>
                {company.name}
              </h1>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary/5 border border-primary/15">
              ...
              <span className="text-sm font-bold text-primary">{t('corporateMgmt.lines')} {linesCount}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <div className="relative">
          <TabsList className="w-full justify-start bg-card border border-border/40 p-1.5 rounded-2xl h-auto flex-wrap shadow-sm gap-1">
            <TabsTrigger value="lines" className="gap-2.5 rounded-xl px-5 py-2.5 text-sm font-semibold data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-lg data-[state=active]:shadow-primary/20 transition-all duration-200" style={{ fontFamily: "'Plus Jakarta Sans', 'IBM Plex Sans Arabic', sans-serif" }}>
              <Truck className="h-4 w-4" />
              {t('corporateMgmt.lines')}
            </TabsTrigger>
            <TabsTrigger value="attendance" className="gap-2.5 rounded-xl px-5 py-2.5 text-sm font-semibold data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-lg data-[state=active]:shadow-primary/20 transition-all duration-200" style={{ fontFamily: "'Plus Jakarta Sans', 'IBM Plex Sans Arabic', sans-serif" }}>
              <CalendarDays className="h-4 w-4" />
              {t('corporateMgmt.attendance')}
            </TabsTrigger>
            <TabsTrigger value="profiles" className="gap-2.5 rounded-xl px-5 py-2.5 text-sm font-semibold data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-lg data-[state=active]:shadow-primary/20 transition-all duration-200" style={{ fontFamily: "'Plus Jakarta Sans', 'IBM Plex Sans Arabic', sans-serif" }}>
              <CreditCard className="h-4 w-4" />
              {t('corporateMgmt.staffFiles')}
            </TabsTrigger>
            {isFinance && (
              <TabsTrigger value="salaries" className="gap-2.5 rounded-xl px-5 py-2.5 text-sm font-semibold data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-lg data-[state=active]:shadow-primary/20 transition-all duration-200" style={{ fontFamily: "'Plus Jakarta Sans', 'IBM Plex Sans Arabic', sans-serif" }}>
                <DollarSign className="h-4 w-4" />
                {t('corporateMgmt.salaries')}
              </TabsTrigger>
            )}
            {isFinance && (
              <TabsTrigger value="invoices" className="gap-2.5 rounded-xl px-5 py-2.5 text-sm font-semibold data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-lg data-[state=active]:shadow-primary/20 transition-all duration-200" style={{ fontFamily: "'Plus Jakarta Sans', 'IBM Plex Sans Arabic', sans-serif" }}>

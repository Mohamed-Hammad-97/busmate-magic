...
export default function ParentAuth() {
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
      setError(t('parentPortal.phoneError'));
      return;
    }
    ...
    if (authInfo.has_password) {
      ...
      if (error) {
        toast({ variant: "destructive", title: t('parentPortal.error'), description: error.message });
      } else {
        setStep("otp");
        startResendTimer();
        toast({ title: t('parentPortal.codeSent'), description: `${t('parentPortal.codeSentTo')} ${phone}` });
      }
    }
  };
  ...
  const handleVerifyOtp = async () => {
    ...
    if (error) {
      toast({ variant: "destructive", title: t('parentPortal.invalidCode'), description: t('parentPortal.tryAgain') });
      setOtp("");
    }
  };
  ...
  const getStepTitle = () => {
    switch (step) {
      case "phone": return t('parentPortal.login');
      case "password": return t('parentPortal.password');
      case "otp": return t('parentPortal.verifyNumber');
    }
  };

  const getStepDescription = () => {
    switch (step) {
      case "phone": return t('parentPortal.phonePlaceholder');
      case "password": return `${t('parentPortal.enterPassword')} ${phone}`;
      case "otp": return `${t('parentPortal.enterOtp')} ${phone}`;
    }
  };
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-gradient-to-br from-primary to-primary/80 items-center justify-center p-12">
        ...
        <div className="relative text-center text-primary-foreground space-y-8 max-w-md">
          ...
          <div>
            ...
            <p className="text-lg text-primary-foreground/80">{t('parentPortal.portalName')}</p>
          </div>
          <div className="space-y-4 pt-8">
            <div className="flex items-center gap-4 p-4 bg-primary-foreground/10 rounded-xl backdrop-blur-sm">
              ...
              <div>
                <div className="font-medium">{t('parentPortal.liveTracking')}</div>
                <div className="text-sm text-primary-foreground/70">{t('parentPortal.liveTrackingDesc')}</div>
              </div>
            </div>
            <div className="flex items-center gap-4 p-4 bg-primary-foreground/10 rounded-xl backdrop-blur-sm">
              <div className="flex-shrink-0 w-12 h-12 bg-primary-foreground/20 rounded-full flex items-center justify-center">
              ...
              <div>
                <div className="font-medium">{t('parentPortal.notifications')}</div>
                <div className="text-sm text-primary-foreground/70">{t('parentPortal.notificationsDesc')}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 relative">
        <div className="w-full max-w-md space-y-8">
          <div className="lg:hidden text-center mb-8">
            ...
            <p className="text-sm text-muted-foreground">{t('parentPortal.portalName')}</p>
          </div>
          ...
          {step === "phone" && (
            <form onSubmit={handlePhoneSubmit} className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="phone" className="text-sm font-medium">{t('parentPortal.phoneLabel')}</Label>
              ...
              <Button type="submit" className="w-full h-12 text-base font-medium shadow-lg shadow-primary/30 hover:shadow-xl hover:shadow-primary/40 transition-all" disabled={isLoading}>
                {isLoading ? <Loader2 className="ml-2 h-5 w-5 animate-spin" /> : (
                  <>
                    {t('parentPortal.continue')}
          ...
          {step === "password" && (
            <form onSubmit={handlePasswordLogin} className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="password" className="text-sm font-medium">{t('parentPortal.password')}</Label>
                <div className="relative">
                  <Lock className="absolute right-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                  <Input id="password" type={showPassword ? "text" : "password"} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="pr-10 pl-10 h-12 bg-muted/50 border-border/50 focus:bg-background transition-colors" autoFocus />
              ...
              <div className="space-y-3">
                <Button type="submit" className="w-full h-12 text-base font-medium shadow-lg shadow-primary/30" disabled={isLoading || !password.trim()}>
                  {isLoading ? <Loader2 className="ml-2 h-5 w-5 animate-spin" /> : t('parentPortal.signIn')}
                </Button>
                <Button type="button" variant="outline" className="w-full h-12" onClick={handleSwitchToOtp} disabled={isLoading}>
                  <Phone className="ml-2 h-5 w-5" />
                  {t('parentPortal.switchToOtp')}
                </Button>
                <Button variant="ghost" className="w-full h-12" onClick={resetToPhone}>
                  <ArrowLeft className="ml-2 h-5 w-5" />
                  {t('parentPortal.changeNumber')}
                </Button>
              </div>
            </form>
          )}
            ...
            <div className="space-y-6">
              <div className="flex flex-col items-center gap-6">
                <div className="bg-muted/50 p-6 rounded-2xl">
                  <InputOTP maxLength={6} value={otp} onChange={(value) => setOtp(value)} onComplete={handleVerifyOtp}>
              ...
              <div className="space-y-3">
                <Button className="w-full h-12 text-base font-medium shadow-lg shadow-primary/30" onClick={handleVerifyOtp} disabled={isLoading || otp.length !== 6}>
                  {isLoading && <Loader2 className="ml-2 h-5 w-5 animate-spin" />}
                  {t('parentPortal.confirm')}
                </Button>
                <Button variant="outline" className="w-full h-12" onClick={async () => {
                  ...
                  if (error) {
                    ...
                    startResendTimer();
                    toast({ title: t('parentPortal.codeResent'), description: `${t('parentPortal.newCodeSent')} ${phone}` });
                    setOtp("");
                  }
                }} disabled={isLoading || resendTimer > 0}>
                  <RefreshCw className="ml-2 h-5 w-5" />
                  {resendTimer > 0
                    ? `${t('parentPortal.resendIn')} ${Math.floor(resendTimer / 60)}:${(resendTimer % 60).toString().padStart(2, '0')}`
                    : t('parentPortal.resendCode')
                  }
                </Button>
                <Button variant="ghost" className="w-full h-12" onClick={resetToPhone}>
                  <ArrowLeft className="ml-2 h-5 w-5" />
